...
########## iNaturalist, imagenet_o,  dtd, places, sun
###places,sun, dtd, iNaturalist, imagenet-o
    ...
    ap.add_argument("--ood_name", type=str, default="imagenet-o")   #iNaturalist, imagenet-o
    ...
    # train_root, val_root = resolve_imagenet_train_val(args.imagenet_root)
    # train_set = datasets.ImageFolder(root=train_root, transform=tfm)
    # id_set = datasets.ImageFolder(root=val_root, transform=tfm)
    # ood_set = build_ood_dataset(args.ood_name, args.ood_dir, tfm)



    # # Minimal cache outputs
    # train_feats_path, train_lbls_path = _paths_train(
    #     args.cache_dir, args.arch, l2_normalize, args.train_max_images, args.seed
    # )
    # id_feats_path = _path_feats(args.cache_dir, "id_val", args.arch, l2_normalize)
    # ood_feats_path = _path_feats(args.cache_dir, args.ood_name, args.arch, l2_normalize)

    # # Cache train feats+labels, id feats, ood feats
    # cache_feats_and_optional_labels_float32(
    #     ds=train_set,
    #     out_feats_path=train_feats_path,
    #     out_labels_path=train_lbls_path,
    #     model=model,
    #     device=device,
    ...
    # Load memmaps
    # train_feats_mm = load_memmap(train_feats_path)
    # train_labels_mm = load_memmap(train_lbls_path)
    # id_feats_mm = load_memmap(id_feats_path)
    # ood_feats_mm = load_memmap(ood_feats_path)
    ...
    for m in methods:
        if m == "subspaces":
            ...
            #     train_feats=train_feats_mm,
            #     train_labels=train_labels_mm,
            #     num_classes=num_classes,
            #     W=W,
            #     b=b,
            #     id_feats=id_feats_mm,
            #     ood_feats=ood_feats_mm,
            #     logits_id=id_logits,
            #     device=device,
            #     energy_keep=args.sub_energy_keep,
            #     use_varfloor=True,          # your current "coeff**2" version
            #     use_log1p=False,
            #     use_centered_for_cos=True,
            #     use_mean_dist=True,
            #     gamma=1.0
            # )
            # id_scores, ood_scores = M.run_method_subspaces(
            #     train_feats=train_feats,
            #     train_labels=train_labels,
            #     id_feats=id_feats,
            #     id_logits=logits_id,
            ...
            # print("beta_star used:", beta_star)
            # print("tau_star used:", tau_star)


            # id_scores, ood_scores, info = M.run_method_subspaces(
            #     train_feats=train_feats,
            #     train_labels=train_labels,
            #     id_feats=id_feats,
            #     ood_feats=ood_feats,
            ...
            #     k_max=0,
            #     top_m=5,                        # e.g. 5
            #     beta=1.0,                          # used if tune_beta=False
            #     tune_beta=True,                # True to calibrate on ID only
            #     gamma=getattr(args, "gamma", 1.0),        # optional scale
            #     use_centered_for_cos=True,
            #     use_ratio_residual=True,   # <-- ratio score switch
            #     device=device,
            #     use_amp=use_amp,
            ...
            # print("beta_used:", info["beta_used"])
            # report_metrics("subspaces_topm", torch.from_numpy(id_scores), torch.from_numpy(ood_scores))
            ...
            # id_scores, ood_scores = M.run_method_mahalanobis(
            #     train_feats=train_feats,
            #     train_labels=train_labels,
            #     id_feats=id_feats,
            #     ood_feats=ood_feats,
            #     num_classes=num_classes,
            # )
            id_scores, ood_scores = M.neco_mahalanobis(
                train_feats=train_feats,
                train_labels=train_labels,
                id_feats=id_feats,
                ood_feats=ood_feats,
            ...
            # id_scores, ood_scores = ensure_ood_higher(np.asarray(id_scores), np.asarray(ood_scores))
            report_metrics("mahalanobis", id_scores, ood_scores)

        elif m == "energy":
            logits_id = logits_from_feats_batched(id_feats_mm, W, b, device=device, batch_size=65536)
            logits_ood = logits_from_feats_batched(ood_feats_mm, W, b, device=device, batch_size=65536)
            id_scores = M.scores_energy_from_logits(logits_id, T=args.energy_T)
            ood_scores = M.scores_energy_from_logits(logits_ood, T=args.energy_T)
            # id_scores, ood_scores = ensure_ood_higher(np.asarray(id_scores), np.asarray(ood_scores))
            ...
            logits_id = logits_from_feats_batched(id_feats_mm, W, b, device=device, batch_size=65536)
            logits_ood = logits_from_feats_batched(ood_feats_mm, W, b, device=device, batch_size=65536)
            id_scores = M.scores_msp_from_logits(logits_id)
            ood_scores = M.scores_msp_from_logits(logits_ood)
            id_scores, ood_scores = ensure_ood_higher(np.asarray(id_scores), np.asarray(ood_scores))
            report_metrics("msp", id_scores, ood_scores)
            ...
            id_scores = M.scores_logit_gate(
                id_feats, W, b, feat_mean, Vk,
                threshold=args.gate_threshold,
                eps=args.gate_eps,
            )
            ood_scores = M.scores_logit_gate(
                ood_feats, W, b, feat_mean, Vk,
                threshold=args.gate_threshold,
                eps=args.gate_eps,
            )
            id_scores, ood_scores = ensure_ood_higher(np.asarray(id_scores), np.asarray(ood_scores))
            report_metrics("logit_gate", id_scores, ood_scores)
            ...
            id_scores, ood_scores = ensure_ood_higher(np.asarray(id_scores), np.asarray(ood_scores))
            report_metrics("kpca_rff", id_scores, ood_scores)
            ...
            print(f"[gradsubspace] using k={k} components.")
            id_scores = M.scores_gradsubspace_pseudo_resid(
                feats=id_feats, W=W, b=b, S=S, mu=mu,
                num_classes=num_classes, eps=args.grad_eps,
            )
            ood_scores = M.scores_gradsubspace_pseudo_resid(
                feats=ood_feats, W=W, b=b, S=S, mu=mu,
                num_classes=num_classes, eps=args.grad_eps,
            )
            id_scores, ood_scores = ensure_ood_higher(np.asarray(id_scores), np.asarray(ood_scores))
            report_metrics("gradsubspace", id_scores, ood_scores)

        elif m == "neco":
            id_scores, ood_scores = M.run_method_neco(
                train_feats=train_feats,
                id_feats=id_feats,
                ood_feats=ood_feats,
                W=W, b=b,
            ...
            s_id, s_ood = M.run_method_vim(
                ...
                seed=args.seed,                 # changes per run
            ...
            id_scores, ood_scores = M.run_deca(
                train_feats=train_feats,
                id_feats=id_feats,
                ood_feats=ood_feats)
            report_metrics("deca", id_scores, ood_scores)

        elif m == "nci":

            # -------------------------
            # Metrics (NCI uses these)
            # -------------------------
            def fpr_at_tpr(id_scores: np.ndarray, ood_scores: np.ndarray, tpr_level: float = 0.95) -> float:
                """
                Robust FPR@TPR: min FPR over all thresholds where TPR >= tpr_level.
                Assumes larger score => more OOD. Labels: ID=0, OOD=1.
                """
                y = np.concatenate(
                    [np.zeros_like(id_scores, dtype=np.int32), np.ones_like(ood_scores, dtype=np.int32)]
                )
                s = np.concatenate([id_scores, ood_scores]).astype(np.float64)
            ...
            def auroc_and_fpr95(id_scores: np.ndarray, ood_scores: np.ndarray) -> Tuple[float, float]:
                ...
                s = np.concatenate([id_scores, ood_scores]).astype(np.float64)
                auroc = float(roc_auc_score(y, s))
                fpr95 = fpr_at_tpr(id_scores, ood_scores, tpr_level=0.95)
                return auroc, fpr95


            def report_metrics_nci(name: str, id_scores_t, ood_scores_t) -> float:
                if isinstance(id_scores_t, torch.Tensor):
                    id_scores = id_scores_t.detach().float().cpu().numpy()
                else:
                    id_scores = np.asarray(id_scores_t, dtype=np.float64)

                if isinstance(ood_scores_t, torch.Tensor):
                    ood_scores = ood_scores_t.detach().float().cpu().numpy()
                else:
                    ood_scores = np.asarray(ood_scores_t, dtype=np.float64)

                # orient: higher => more OOD
            ...
            # -------------------------
            # ImageNet ID-val loader
            # -------------------------
            def make_imagenet1k_id_val_loader_only_for_nci(
                train_set,
                *,
                batch_size: int,
                num_workers: int,
                ...
                """
                NCI-only: build an ImageNet-1K ID validation loader by subsampling from ImageNet train split.
                Uses the SAME transform already attached to train_set.
                """
                n = len(train_set)
                val_size = int(min(val_size, n))

                rng = np.random.RandomState(seed)
                idx = rng.choice(n, size=val_size, replace=False).tolist()

                id_val_set = Subset(train_set, idx)
                ...
                return id_val_set, id_val_loader
            ...
            def extract_feats_backbone(
                ...
                *,
                ...
                """
                Extract features for a model that may return:
                - feats
                - (feats, logits)
            ...
            def extract_gaussian_feats_like_id(
                ...
                *,
                ...
                """
                Create Gaussian noise inputs with the same batch shape as ID batches and extract features.
                Returns: [N, D] on CPU, float32.
                """
                model.eval()
                outs = []
            ...
            # One-call NCI runner
            # -------------------------
            def run_nci_imagenet1k_resnet50(
                *,
                model: torch.nn.Module,
                device: torch.device,
                train_set,                 # ImageFolder(train_root, transform=tfm)
                id_feats: torch.Tensor,    # memmap-backed torch tensor [N_id, D]
                ood_feats: torch.Tensor,   # memmap-backed torch tensor [N_ood, D]
                train_feats: torch.Tensor, # memmap-backed torch tensor [N_train, D]
                W: torch.Tensor,           # [C, D] (or np.ndarray, but torch preferred)
                b: Optional[torch.Tensor], # [C] or None
                use_amp: bool,
                batch_size: int,
                workers: int,
                seed: int,
                val_size: int = 5000,
                varpi_grid: Sequence[float] = (1e-4, 1e-3, 1e-2, 1e-1),
                p_norm: int = 1,
            ):
                """
                End-to-end NCI for ImageNet-1K + ResNet50 backbone wrapper:
                1) subsample ImageNet train as ID-val
                2) extract ID-val feats and Gaussian feats
                3) compute mu_g from cached train_feats
                4) tune varpi on ID-val vs Gaussian
                5) evaluate on (id_feats vs ood_feats) using best varpi

                Returns:
                best_varpi, best_auc_val, (auroc_test, fpr95_test), (s_id_test, s_ood_test)
                """
                # ---- 0) ensure W,b,mu_g are on CPU tensors for your nci_scores_batched (most implementations accept torch)
                if not isinstance(W, torch.Tensor):
                    W = torch.from_numpy(np.asarray(W))
                if b is not None and not isinstance(b, torch.Tensor):
                    b = torch.from_numpy(np.asarray(b))

                # ---- 1) ID-val loader from ImageNet train subset
                _, id_val_loader = make_imagenet1k_id_val_loader_only_for_nci(
                    train_set=train_set,
                    batch_size=batch_size,
                    num_workers=workers,
                    seed=seed,
                ...
                # ---- 2) Extract feats for tuning
                id_val_feats = extract_feats_backbone(model, id_val_loader, device, use_amp=use_amp)
                gau_val_feats = extract_gaussian_feats_like_id(model, id_val_loader, device, use_amp=use_amp)

                # ---- 3) Global mean from train feats
                mu_g = M.compute_mu_global(train_feats)  # expects torch.Tensor [N_train, D] -> [D]

                # ---- 4) Tune varpi
                best_varpi, best_auc = None, -1.0
                for v in varpi_grid:
                    _, s_id_val = M.nci_scores_batched(id_val_feats, W, b, mu_g, alpha=float(v), p_norm=p_norm)
                    _, s_gau_val = M.nci_scores_batched(gau_val_feats, W, b, mu_g, alpha=float(v), p_norm=p_norm)

                    auc = report_metrics_nci(f"NCI (val ID vs Gaussian) varpi={v:g}", s_id_val, s_gau_val)
                    if auc > best_auc:
                ...
                # ---- 5) Final evaluation
                _, s_id_test = M.nci_scores_batched(id_feats, W, b, mu_g, alpha=best_varpi, p_norm=p_norm)
                _, s_ood_test = M.nci_scores_batched(ood_feats, W, b, mu_g, alpha=best_varpi, p_norm=p_norm)

                # metrics (for returning)
                id_np = s_id_test.detach().float().cpu().numpy() if isinstance(s_id_test, torch.Tensor) else np.asarray(s_id_test)
                ood_np = s_ood_test.detach().float().cpu().numpy() if isinstance(s_ood_test, torch.Tensor) else np.asarray(s_ood_test)
                auroc_test, fpr95_test = auroc_and_fpr95(id_np, ood_np)

                report_metrics_nci("nci", s_id_test, s_ood_test)
                                # Final print + return
                print(f"\n=== FINAL (ID-val vs OOD) ===")
                print(f"best_varpi = {best_varpi:g}")
                print(f"AUROC (OOD positive):      {auroc_test:.4f}")
                print(f"FPR@95%TPR (OOD positive): {fpr95_test:.4f}")

                return best_varpi, best_auc, (auroc_test, fpr95_test), (s_id_test, s_ood_test)
            best_varpi, best_auc_val, (auroc, fpr95), (s_id, s_ood) = run_nci_imagenet1k_resnet50(
                model=model,
                device=device,
                train_set=train_set,
                id_feats=id_feats,
            ...
            print(f"\nReturned: AUROC={auroc:.4f}, FPR95={fpr95:.4f}, best_varpi={best_varpi:g}")
