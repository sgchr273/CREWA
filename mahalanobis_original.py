...
def compute_mahalanobis_scores(train_feats, train_labels, query_feats, device='cuda'):
    """
    Computes Mahalanobis OOD score = max_c [ -(f - mu_c)^T Sigma^{-1} (f - mu_c) ]
    (higher = more in-distribution)

    Args:
        train_feats:  (N_train, D) tensor  — ID train features
        train_labels: (N_train,)   tensor  — ID train labels
        query_feats:  (N_query, D) tensor  — ID test or OOD features
    Returns:
        scores: (N_query,) numpy array
    """
    ...
    # 2. Compute pooled within-class covariance (tied covariance)
    centered = []
    for c in classes:
        mask = train_labels == c
        centered.append(train_feats[mask] - class_means[c])
    ...
    precision = cov_estimator.precision_  # (D, D) inverse covariance

    # 3. Score each query sample: max over classes of negative Mahal distance
    scores = np.zeros(len(query_feats))
    for i, f in enumerate(query_feats):
        dists = []
        for c in classes:
            diff = f - class_means[c]                         # (D,)
            dist = diff @ precision @ diff                    # scalar
            dists.append(-dist)                               # negative distance = higher is more ID
        scores[i] = -np.max(dists)
...
id_scores  = compute_mahalanobis_scores(train_feats, train_labels, id_feats,  device)
ood_scores = compute_mahalanobis_scores(train_feats, train_labels, ood_feats, device)

# AUROC (label 1 = ID, 0 = OOD)
...
print(f"Mahalanobis  AUROC: {auroc:.4f}  FPR@95: {fpr95:.4f}")
